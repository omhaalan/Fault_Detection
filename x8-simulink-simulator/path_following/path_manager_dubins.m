...
%
function out = path_manager_dubins(states,waypoints,P,start_of_simulation)
...
%   NN = NN + 1 + 5*P.size_waypoint_array;
  ...
  if min(min(waypoints==waypoints_old))==0,
      ...
      end_node   = [waypoints(1:4,ptr_b)', 0, 0];
        dubinspath = struct;
        dubinspath.ps   = zeros(3,1);
        dubinspath.chis = zeros(1,1);
        dubinspath.pe   = zeros(3,1);
        dubinspath.chie = zeros(1,1);
        dubinspath.R    = zeros(1,1);
        dubinspath.L    = zeros(1,1);
        dubinspath.cs   = zeros(3,1);
        dubinspath.lams = zeros(1,1);
        dubinspath.ce   = zeros(3,1);
        dubinspath.lame = zeros(1,1);
        dubinspath.w1   = zeros(3,1);
        dubinspath.q1   = zeros(1,1);
        dubinspath.w2   = zeros(3,1);
        dubinspath.w3   = zeros(3,1);
        dubinspath.q3   = zeros(1,1);
        while all(start_node == end_node)
           ptr_b = ptr_b +1;
            
           end_node   = [waypoints(1:4,ptr_b)', 0, 0];  
        end
      dubinspath = dubinsParameters(start_node, end_node, P.R_min,dubinspath);  
      flag_need_new_waypoints = 0;
      flag_first_time_in_state = 1;
  end
