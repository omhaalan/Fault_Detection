function [output] = forces_trim(eta,ny,quat,control,wind,P,time)
    %%  forces.m
    % NOTE: This function is virtually identical to forces.m except that 
    % icing is complete deactivated here. 
    %
    ...
    %% relative velocity
    wind_b = Rzyx(phi,theta,psi)'*wind(1:3);
    vel_r = vel-wind_b;
    u_r = vel_r(1);
    v_r = vel_r(2);
    w_r = vel_r(3);
    ...
    %% Change Lift coefficient
    C_L_D = P.C_L_alpha;
    %         if time>300 && time<400
    %             P.C_L_alpha = P.C_L_alpha * (1-0.2/100*(time-300));
    %             P.alpha0 = P.alpha0 * (1-0.2/100*(time-300));
    %             P.C_D_p = P.C_D_p * (1+1/100*(time-300));
    %         elseif time>=400
    %             P.C_L_alpha = P.C_L_alpha * 0.8;
    %             P.alpha0 = P.alpha0 * 0.8;
    %             P.C_D_p = P.C_D_p * 2;
    % %         elseif time>=600 && time<650
    % %             P.C_L_alpha = P.C_L_alpha * (0.8+0.2/50*(time-600));
    % %             P.alpha0 = P.alpha0 * (0.8+0.2/50*(time-600));
    % %             P.C_D_p = P.C_D_p * (2-1/50*(time-600));
    %         end
    
    C_out = [P.C_L_alpha;P.C_L_0;P.C_D_p];
    %% Longitudinal mode
    ...
    C_L_alpha = C_L(alpha,0);
    % compute aero lift force
    f_lift_s = 0.5*P.rho*Va^2*P.S_wing*(C_L_alpha + P.C_L_q*P.c/(2*Va)*q + -0.3+P.C_L_delta_e*elevator);     %eqn 4.6 in B&M
    
    % drag
    %     C_D_alpha = P.C_D_p + (1-sigma)*(P.C_L_0 + C_L_D*alpha)^2/(pi*P.e*(P.b^2/P.S_wing)) + sigma*2*sign(alpha)*sin(alpha)^3;
    C_D_alpha = C_D(alpha,0);
    f_drag_s = 0.5*P.rho*Va^2*P.S_wing*(C_D_alpha + P.C_D_q*P.c/(2*Va)*q + P.C_D_delta_e*abs(elevator));     %eqn 4.7 in B&M
    
    % pitch moment
    %     m_a = (P.C_m_0 + (P.C_m_alpha)*alpha);
    m_a = 0.14+C_m(alpha,0);
    %     m_a = ((1-sigma)*(P.C_m_0 + P.C_m_alpha*alpha) + sigma*(P.C_m_flatplate*2*sign(alpha)*sin(alpha)^2));
    ...
    output = [Force; Torque; Va; alpha; beta; course; crab; gamma; gamma_a; wind_b;T_prop(1);C_out];
end